  ...
  testGroup
    ...
    , testCase "sin(x)^2 * cos(x)^2 contracts to (1/8)-(cos(4*x)/8)" $ do
        ...
        case contractTrig (simplifyOrFail expr) of
          Right out -> out @?= simplifyOrFail expected
          Left e    -> fail $ "contractTrig failed: " ++ show e
    , testCase "(sin(x) + cos(y)) * cos(y) contracts to sin(x + y)/2 + sin(x - y)/2 + 1/2 + cos(2*y)/2" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            y = mkSymbol "y" :: UnsimplifiedExpr
        ...
        case contractTrig (simplifyOrFail expr) of
          Right out -> out @?= simplifyOrFail expected
          Left e    -> fail $ "contractTrig failed: " ++ show e
    , testCase "(cos(x) + sin(x)) ^ 4 contracts to 3/2 - 1/2*cos (4*x) + 3*sin(0) + 1/2*sin((-2)*x) + 5/2*sin (2*x)" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            expr = (cos x + sin x) ** 4
            expected = (3 / 2) - (1 / 2) * cos (4 * x) + (3 * sin 0) + (1 / 2) * sin ((-2) * x) + (5 / 2) * sin (2 * x)
        case contractTrig (simplifyOrFail expr) of
          Right out -> out @?= simplifyOrFail expected
          Left e    -> fail $ "contractTrig failed: " ++ show e
    , testCase "(cos(x) + sin(x))^4 + (cos(x) - sin(x))^4 + cos(4*x) - 3 contracts to 0" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            expr = (cos x + sin x) ** 4 + (cos x - sin x) ** 4 + cos (4 * x) - 3
            expected = 0 :: UnsimplifiedExpr
        case contractTrig (simplifyOrFail expr) of
          Right out -> out @?= simplifyOrFail expected
          Left e    -> fail $ "contractTrig failed: " ++ show e
    , testCase "sin(x) + sin(y) - 2*sin(x/2 + y/2)*cos(x/2 - y/2) contracts to 0" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            y = mkSymbol "y" :: UnsimplifiedExpr
            expr = sin x + sin y - 2 * sin (x / 2 + y / 2) * cos (x / 2 - y / 2)
            expected = 0 :: UnsimplifiedExpr
        case contractTrig (simplifyOrFail expr) of
          Right out -> out @?= simplifyOrFail expected
          Left e    -> fail $ "contractTrig failed: " ++ show e
...
-- simplifyTrigTests :: TestTree
-- simplifyTrigTests =
--   testGroup
--     "simplifyTrig"
--     [ testCase "sin(x)^3 +cos(x + pi/6)^3 - sin(x + pi/3)^3 + (3*sin(3*x) / 4) simplifies to 0" $ do
--         let x = mkSymbol "x" :: UnsimplifiedExpr
--             expr = sin x ** 3 + cos (x + pi / 6) ** 3 - sin (x + pi / 3) ** 3 + (3 * sin (3 * x) / 4)
--             expected = 0 :: UnsimplifiedExpr
--         case simplifyTrig (simplifyOrFail expr) of
--           Right out -> out @?= simplifyOrFail expected
--           Left e -> fail $ "simplifyTrig failed: " ++ show e,
--       testCase "(sin(x) + sin(3*x) + sin(5*x) +sin(7*x)) / (cos(x) + cos(3*x) + cos(5*x) +cos(7*x)) - tan(4*x) simplifies to 0" $ do
--         let x = mkSymbol "x" :: UnsimplifiedExpr
--             expr = (sin x + sin (3 * x) + sin (5 * x) + sin (7 * x)) / (cos x + cos (3 * x) + cos (5 * x) + cos (7 * x)) - tan (4 * x)
--             expected = 0 :: UnsimplifiedExpr
--         case simplifyTrig (simplifyOrFail expr) of
--           Right out -> out @?= simplifyOrFail expected
--           Left e -> fail $ "simplifyTrig failed: " ++ show e,
--       testCase "sin(x)*cos(x)*sec(x) simplifies to sin(x)" $ do
--         let x = mkSymbol "x" :: UnsimplifiedExpr
--             expr = sin x * cos x * Sec' x
--             expected = sin x
--         case simplifyTrig (simplifyOrFail expr) of
--           Right out -> out @?= simplifyOrFail expected
--           Left e -> fail $ "simplifyTrig failed: " ++ show e,
--       testCase "tan(x)*sin(x) + sec(x)*cos(x)^2 simplifies to 1/cos(x)" $ do
--         let x = mkSymbol "x" :: UnsimplifiedExpr
--             expr = Tan' x * sin x + Sec' x * cos x ** 2
--             expected = 1 / cos x
--         case simplifyTrig (simplifyOrFail expr) of
--           Right out -> out @?= simplifyOrFail expected
--           Left e -> fail $ "simplifyTrig failed: " ++ show e,
--       testCase "tan(x/2) - sin(x)/(1 + cos(x)) simplifies to 0" $ do
--         let x = mkSymbol "x" :: UnsimplifiedExpr
--             expr = tan (x / 2) - sin x / (1 + cos x)
--             expected = 0 :: UnsimplifiedExpr
--         case simplifyTrig (simplifyOrFail expr) of
--           Right out -> out @?= simplifyOrFail expected
--           Left e -> fail $ "simplifyTrig failed: " ++ show e,
--       testCase "sin(15*pi/16)^2 + cos(pi/16)^2 - 1 simplifies to 0" $ do
--         let expr = sin (15 * pi / 16) ** 2 + cos (pi / 16) ** 2 - 1
--             expected = 0 :: UnsimplifiedExpr
--         case simplifyTrig (simplifyOrFail expr) of
--           Right out -> out @?= simplifyOrFail expected
--           Left e -> fail $ "simplifyTrig failed: " ++ show e,
--       testCase "sin(x^2 - 1)^2 + cos(1 - x^2)^2 - 1 simplifies to 0" $ do
--         let x = mkSymbol "x" :: UnsimplifiedExpr
--             expr = sin (x ** 2 - 1) ** 2 + cos (1 - x ** 2) ** 2 - 1
--             expected = 0 :: UnsimplifiedExpr
--         case simplifyTrig (simplifyOrFail expr) of
--           Right out -> out @?= simplifyOrFail expected
--           Left e -> fail $ "simplifyTrig failed: " ++ show e,
--       testCase "sin(x + 1/x)^2 + cos((x^2 + 1)/x)^2 - 1 simplifies to 0" $ do
--         let x = mkSymbol "x" :: UnsimplifiedExpr
--             expr = sin (x + 1 / x) ** 2 + cos ((x ** 2 + 1) / x) ** 2 - 1
--             expected = 0 :: UnsimplifiedExpr
--         case simplifyTrig (simplifyOrFail expr) of
--           Right out -> out @?= simplifyOrFail expected
--           Left e -> fail $ "simplifyTrig failed: " ++ show e,
--       testCase "sin(x * (1 + pi/(6*x))) - sqrt(3)/2*sin(x) - 1/2*sin(x) simplifies to 0" $ do
--         let x = mkSymbol "x" :: UnsimplifiedExpr
--             expr = sin (x * (1 + pi / (6 * x))) - (sqrt (3 :: UnsimplifiedExpr) / 2) * sin x - (1 / 2) * sin x
--             expected = 0 :: UnsimplifiedExpr
--         case simplifyTrig (simplifyOrFail expr) of
--           Right out -> out @?= simplifyOrFail expected
--           Left e -> fail $ "simplifyTrig failed: " ++ show e,
--       testCase "sin((sin(x)^2 + cos(x)^2) * (x + pi/6)) - sqrt(3)/2*sin(x) - 1/2*sin(x) simplifies to 0" $ do
--         let x = mkSymbol "x" :: UnsimplifiedExpr
--             expr = sin ((sin x ** 2 + cos x ** 2) * (x + pi / 6)) - (sqrt (3 :: UnsimplifiedExpr) / 2) * sin x - (1 / 2) * sin x
--             expected = 0 :: UnsimplifiedExpr
--         case simplifyTrig (simplifyOrFail expr) of
--           Right out -> out @?= simplifyOrFail expected
--           Left e -> fail $ "simplifyTrig failed: " ++ show e,
--       testCase "8*cos(2*pi/7)^3 + 4*cos(2*pi/7)^2 - 4*cos(2*pi/7) - 1 simplifies to 0" $ do
--         let expr = 8 * cos (2 * pi / 7) ** 3 + 4 * cos (2 * pi / 7) ** 2 - 4 * cos (2 * pi / 7) - 1
--             expected = 0 :: UnsimplifiedExpr
--         case simplifyTrig (simplifyOrFail expr) of
--           Right out -> out @?= simplifyOrFail expected
--           Left e -> fail $ "simplifyTrig failed: " ++ show e
