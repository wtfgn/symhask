...
checkMonomial expr expected = isMonomialSv "x" expr @?= expected

checkPolynomial :: SimplifiedExpr -> Bool -> IO ()
checkPolynomial expr expected = isPolynomialSv "x" expr @?= expected
...
degreeMonomialTests =
  testGroup
    ...
    [ testCase "constants have degree 0" $ do
        ...
        degreeMonomialSv "x" (simplifyOrFail expr1) @?= Just 0
        degreeMonomialSv "x" (simplifyOrFail expr2) @?= Just 0
    , testCase "zero is Undefined" $ do
        let expr = mkNumber 0 :: UnsimplifiedExpr
        degreeMonomialSv "x" (simplifyOrFail expr) @?= Nothing
    , testCase "the variable itself has degree 1" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
        degreeMonomialSv "x" (simplifyOrFail x) @?= Just 1
    , testCase "powers of the variable" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
        degreeMonomialSv "x" (simplifyOrFail (x ** (2 :: UnsimplifiedExpr))) @?= Just 2
        degreeMonomialSv "x" (simplifyOrFail (x ** (3 :: UnsimplifiedExpr))) @?= Just 3
    , testCase "products of constants and powers" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
        degreeMonomialSv "x" (simplifyOrFail (mkNumber 2 * x)) @?= Just 1
        degreeMonomialSv "x" (simplifyOrFail (mkNumber 3 * (x ** (2 :: UnsimplifiedExpr)))) @?= Just 2
    , testCase "different variables are Undefined" $ do
        let y = mkSymbol "y" :: UnsimplifiedExpr
        degreeMonomialSv "x" (simplifyOrFail y) @?= Nothing
    , testCase "sums are Undefined" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
        degreeMonomialSv "x" (simplifyOrFail (x + 1)) @?= Nothing
    ]
...
degreeTests =
  testGroup
    ...
    [ testCase "degree of polynomial 3x^2 + 4x + 5 is 2" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            expr = (mkNumber 3 * (x ** (2 :: UnsimplifiedExpr))) + (mkNumber 4 * x) + 5
        degreeSv "x" (simplifyOrFail expr) @?= Just 2
    , testCase "degree of monomial 2x^3 is 3" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
        degreeSv "x" (simplifyOrFail (mkNumber 2 * (x ** (3 :: UnsimplifiedExpr)))) @?= Just 3
    , testCase "degree of constant is 0" $ do
        degreeSv "x" (simplifyOrFail (mkNumber 5 :: UnsimplifiedExpr)) @?= Just 0
    , testCase "degree of product of sums is Undefined" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            expr = (x + 1) * (x + 3)
        degreeSv "x" (simplifyOrFail expr) @?= Nothing
    , testCase "polynomial with varying degrees" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            expr = x + (x ** (5 :: UnsimplifiedExpr)) + (mkNumber 2 * (x ** (3 :: UnsimplifiedExpr)))
        degreeSv "x" (simplifyOrFail expr) @?= Just 5
    , testCase "sum with non-polynomial term is Undefined" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
        degreeSv "x" (simplifyOrFail (x + mkFunction "sin" (x :| []))) @?= Nothing
    ]
...
coefficientTests =
  testGroup
    ...
    [ testCase "coefficient of x^1 in x^2 + 3x + 5 is 3" $ do
        ...
        coefficientSv "x" 1 (simplifyOrFail expr) @?= Right (mkNumber 3)
    , testCase "coefficient of x^4 in 2x^3 + 3x is 0" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            expr = (mkNumber 2 * (x ** (3 :: UnsimplifiedExpr))) + (mkNumber 3 * x)
        coefficientSv "x" 4 (simplifyOrFail expr) @?= Right (mkNumber 0)
    , testCase "coefficient of x^0 in 3 is 3" $ do
        coefficientSv "x" 0 (simplifyOrFail (mkNumber 3 :: UnsimplifiedExpr)) @?= Right (mkNumber 3)
    , testCase "coefficient of x^2 in 2*x^2/3 + x + 1 is 2/3" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            expr = (mkFraction 2 3 * (x ** (2 :: UnsimplifiedExpr))) + x + 1
        coefficientSv "x" 2 (simplifyOrFail expr) @?= simplify (mkFraction 2 3)
    , testCase "coefficient of x^2 in (x + 1)(x + 3) returns error" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            expr = (x + 1) * (x + 3)
        case coefficientSv "x" 2 (simplifyOrFail expr) of
          Left _  -> pure ()
...
leadingCoefficientTests =
  testGroup
    ...
    [ testCase "leading coefficient of x^2 + 3*x + 5 is 1" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            expr = (x ** (2 :: UnsimplifiedExpr)) + (mkNumber 3 * x) + 5
        leadingCoefficientSv "x" (simplifyOrFail expr) @?= Right (mkNumber 1)
    , testCase "leading coefficient of 3 is 3" $ do
        leadingCoefficientSv "x" (simplifyOrFail (mkNumber 3 :: UnsimplifiedExpr)) @?= Right (mkNumber 3)
    , testCase "leading coefficient of 3*x^2 + 4*x + 5 is 3" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            expr = (mkNumber 3 * (x ** (2 :: UnsimplifiedExpr))) + (mkNumber 4 * x) + 5
        leadingCoefficientSv "x" (simplifyOrFail expr) @?= Right (mkNumber 3)
    , testCase "leading coefficient of non-polynomial returns error" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            expr = (x + 1) * (x + 3)
        case leadingCoefficientSv "x" (simplifyOrFail expr) of
          Left _  -> pure ()
          Right _ -> fail "Expected Left error for non-polynomial"
    ]
...
monomialGpeTests =
  testGroup
    ...
    [ testCase "a*x^2*y^2 is GME in {x, y}" $ do
        ...
        isMonomialGpe (HS.fromList [simplifyOrFail x, simplifyOrFail y]) (simplifyOrFail expr) @?= True
    , testCase "x^2 + y^2 is not GME in {x, y}" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            y = mkSymbol "y" :: UnsimplifiedExpr
            expr = (x ** (2 :: UnsimplifiedExpr)) + (y ** (2 :: UnsimplifiedExpr))
        isMonomialGpe (HS.fromList [simplifyOrFail x, simplifyOrFail y]) (simplifyOrFail expr) @?= False
    , testCase "x is GME in {x}" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
        isMonomialGpe (HS.fromList [simplifyOrFail x]) (simplifyOrFail x) @?= True
    , testCase "x^3 is GME in {x}" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            expr = x ** (3 :: UnsimplifiedExpr)
        isMonomialGpe (HS.fromList [simplifyOrFail x]) (simplifyOrFail expr) @?= True
    , testCase "3 is GME in {x} (free of x)" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
        isMonomialGpe (HS.fromList [simplifyOrFail x]) (simplifyOrFail (mkNumber 3 :: UnsimplifiedExpr)) @?= True
    ]
  ...
  testGroup
    ...
    [ testCase "x^2 + y^2 is GPE in {x, y}" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            y = mkSymbol "y" :: UnsimplifiedExpr
            expr = (x ** (2 :: UnsimplifiedExpr)) + (y ** (2 :: UnsimplifiedExpr))
        isPolynomialGpe (HS.fromList [simplifyOrFail x, simplifyOrFail y]) (simplifyOrFail expr) @?= True
    , testCase "sin^2(x) + 2*sin(x) + 3 is GPE in {sin(x)}" $ do
        ...
        isPolynomialGpe (HS.fromList [simplifyOrFail sinx]) (simplifyOrFail expr) @?= True
    , testCase "x/y + 2*y is not GPE in {x, y}" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            y = mkSymbol "y" :: UnsimplifiedExpr
            expr = (x / y) + (mkNumber 2 * y)
        isPolynomialGpe (HS.fromList [simplifyOrFail x, simplifyOrFail y]) (simplifyOrFail expr) @?= False
    , testCase "(x + 1)*(x + 3) is not GPE in {x}" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            expr = (x + 1) * (x + 3)
        isPolynomialGpe (HS.fromList [simplifyOrFail x]) (simplifyOrFail expr) @?= False
    , testCase "3 is GPE in {x}" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
        isPolynomialGpe (HS.fromList [simplifyOrFail x]) (simplifyOrFail (mkNumber 3 :: UnsimplifiedExpr)) @?= True
    , testCase "a + b is GPE in {a + b}" $ do
        let a = mkSymbol "a" :: UnsimplifiedExpr
            b = mkSymbol "b" :: UnsimplifiedExpr
            expr = a + b
        isPolynomialGpe (HS.fromList [simplifyOrFail expr]) (simplifyOrFail expr) @?= True
    , testCase "3*w*x^2*y^3*z^4 is GPE in {x, z}" $ do
        ...
        isPolynomialGpe (HS.fromList [simplifyOrFail x, simplifyOrFail z]) (simplifyOrFail expr) @?= True
    , testCase "a*(x^2 + 1)^2 + (x^2 + 1) is GPE in {x^2 + 1}" $ do
        let a = mkSymbol "a" :: UnsimplifiedExpr
            x = mkSymbol "x" :: UnsimplifiedExpr
            x2Plus1 = (x ** (2 :: UnsimplifiedExpr)) + 1
            expr = a * (x2Plus1 ** (2 :: UnsimplifiedExpr)) + x2Plus1
        isPolynomialGpe (HS.fromList [simplifyOrFail x2Plus1]) (simplifyOrFail expr) @?= True
    ...
    , testCase "y^2*(y^4 + 1) is GPE in {y^2}" $ do
        ...
        isPolynomialGpe (HS.fromList [simplifyOrFail (y ** (2 :: UnsimplifiedExpr))]) (simplifyOrFail expr) @?= True
    , testCase "2*(x^2)^2 + 3*(x^2) is GPE in {x^2}" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            x2 = x ** (2 :: UnsimplifiedExpr)
            expr = mkNumber 2 * (x2 ** (2 :: UnsimplifiedExpr)) + mkNumber 3 * x2
        isPolynomialGpe (HS.fromList [simplifyOrFail x2]) (simplifyOrFail expr) @?= True
    ]
  ...
  testGroup
    ...
    [ testCase "degree of a*x^2*y^2 in {x,y} is 4" $ do
        let a = mkSymbol "a" :: UnsimplifiedExpr
            ...
            y = mkSymbol "y" :: UnsimplifiedExpr
            expr = a * (x ** (2 :: UnsimplifiedExpr)) * (y ** (2 :: UnsimplifiedExpr))
        degreeGpe (HS.fromList [simplifyOrFail x, simplifyOrFail y]) (simplifyOrFail expr) @?= Right (Just 4)
    , testCase "degree of 3*w*x^2*y^3*z^4 in {x, z} is 6" $ do
        let w = mkSymbol "w" :: UnsimplifiedExpr
            ...
            z = mkSymbol "z" :: UnsimplifiedExpr
            expr = mkNumber 3 * w * (x ** (2 :: UnsimplifiedExpr)) * (y ** (3 :: UnsimplifiedExpr)) * (z ** (4 :: UnsimplifiedExpr))
        degreeGpe (HS.fromList [simplifyOrFail x, simplifyOrFail z]) (simplifyOrFail expr) @?= Right (Just 6)
    , testCase "degree of x^2 + y^2 in {x,y} is 2" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            y = mkSymbol "y" :: UnsimplifiedExpr
            expr = (x ** (2 :: UnsimplifiedExpr)) + (y ** (2 :: UnsimplifiedExpr))
        degreeGpe (HS.fromList [simplifyOrFail x, simplifyOrFail y]) (simplifyOrFail expr) @?= Right (Just 2)
    , testCase "degree of a*sin(x)^2 + b*sin(x) + c in {sin(x)} is 2" $ do
        let a = mkSymbol "a" :: UnsimplifiedExpr
            ...
            x = mkSymbol "x" :: UnsimplifiedExpr
            expr = a * (mkFunction "sin" (x :| []) ** (2 :: UnsimplifiedExpr)) + mkNumber 2 * b * mkFunction "sin" (x :| []) + mkNumber 3 * c
        degreeGpe (HS.fromList [simplifyOrFail (mkFunction "sin" (x :| []))]) (simplifyOrFail expr) @?= Right (Just 2)
    , testCase "degree of 2*x^2*y*z^3 + w*x*z^6 in {x, z} is 7" $ do
        ...
        degreeGpe (HS.fromList [simplifyOrFail x, simplifyOrFail z]) (simplifyOrFail expr) @?= Right (Just 7)
    , testCase "degree of zero monomial is -infinity (Nothing)" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
        degreeGpe (HS.fromList [simplifyOrFail x]) (simplifyOrFail (mkNumber 0 :: UnsimplifiedExpr)) @?= Right Nothing
    , testCase "degreeGpe errors on non-GPE" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            expr = (x + 1) * (x + 3)
        case degreeGpe (HS.fromList [simplifyOrFail x]) (simplifyOrFail expr) of
    ...
    , testCase "degree of a*(x^2 + 1)^2 + (x^2 + 1) in {x^2 + 1} is 2" $ do
        let a = mkSymbol "a" :: UnsimplifiedExpr
            ...
            x2Plus1 = (x ** (2 :: UnsimplifiedExpr)) + 1
            expr = a * (x2Plus1 ** (2 :: UnsimplifiedExpr)) + x2Plus1
        degreeGpe (HS.fromList [simplifyOrFail x2Plus1]) (simplifyOrFail expr) @?= Right (Just 2)
    , testCase "degree of 2*(x^2)^2 + 3*(x^2) in {x^2} is 1" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            x2 = x ** (2 :: UnsimplifiedExpr)
            expr = mkNumber 2 * (x2 ** (2 :: UnsimplifiedExpr)) + mkNumber 3 * x2
        degreeGpe (HS.fromList [simplifyOrFail x2]) (simplifyOrFail expr) @?= Right (Just 1)
    ]
  ...
  testGroup
    ...
    [ testCase "coefficient of a*x^2 + b*x + c in x^2 is a" $ do
        let a = mkSymbol "a" :: UnsimplifiedExpr
            ...
            x = mkSymbol "x" :: UnsimplifiedExpr
            expr = (a * (x ** (2 :: UnsimplifiedExpr))) + (b * x) + c
        coefficientGpe (simplifyOrFail x) 2 (simplifyOrFail expr) @?= Right (simplifyOrFail a)
    , testCase "coefficient of 3*x*y^2 + 5*x^2*y + 7*x + 9 in x is 3*y^2 + 7" $ do
        ...
        coefficientGpe (simplifyOrFail x) 1 (simplifyOrFail expr) @?= Right (simplifyOrFail expected)
    , testCase "coefficient of 3*x*y^2 + 5*x^2*y + 7*x + 9 in x^3 is 0" $ do
        let x = mkSymbol "x"
            y = mkSymbol "y"
            expr = mkNumber 3 * x * (y ** (2 :: UnsimplifiedExpr)) + mkNumber 5 * (x ** (2 :: UnsimplifiedExpr)) * y + mkNumber 7 * x + mkNumber 9
            expected = mkNumber 0
        coefficientGpe (simplifyOrFail x) 3 (simplifyOrFail expr) @?= Right (simplifyOrFail expected)
    , testCase "coefficient of x^3 when absent is 0" $ do
        let x = mkSymbol "x"
            y = mkSymbol "y"
            expr = mkNumber 3 * x * (y ** (2 :: UnsimplifiedExpr)) + mkNumber 5 * (x ** (2 :: UnsimplifiedExpr)) * y + mkNumber 7 * x + mkNumber 9
        coefficientGpe (simplifyOrFail x) 3 (simplifyOrFail expr) @?= Right (mkNumber 0)
    , testCase "coefficient of 3*x*y^2 + 5*x^2*y + 7*x + 9 in x*y^2 is 3" $ do
        let x = mkSymbol "x"
            y = mkSymbol "y"
            expr = mkNumber 3 * x * (y ** (2 :: UnsimplifiedExpr)) + mkNumber 5 * (x ** (2 :: UnsimplifiedExpr)) * y + mkNumber 7 * x + mkNumber 9
            expected = mkNumber 3
        case coefficientGpe (simplifyOrFail x) 1 (simplifyOrFail expr)  of
          Right inner -> coefficientGpe (simplifyOrFail y) 2 inner  @?= Right (simplifyOrFail expected)
    ...
    , testCase "coefficient of 3*sin(x)*x^2 + 2*log(x)*x +4 in log(x)*x is 2" $ do
        ...
        case coefficientGpe (simplifyOrFail logX) 1 (simplifyOrFail expr)  of
          Right inner -> coefficientGpe (simplifyOrFail x) 1 inner  @?= Right (mkNumber 2)
          Left _ -> fail "Expected Right from first coefficientGpe"
    , testCase "coefficient of x in x is 1" $ do
        let x = mkSymbol "x"
        coefficientGpe x 1 (simplifyOrFail x) @?= Right (mkNumber 1)
    , testCase "coefficientGpe returns error for non-polynomial term" $ do
        let x = mkSymbol "x"
            sinX = mkFunction "sin" (x :| []) :: UnsimplifiedExpr
            logX = mkFunction "log" (x :| []) :: UnsimplifiedExpr
            expr = 3 * sinX * (x ** 2) + 2 * logX * x + 4
        case coefficientGpe x 2 (simplifyOrFail expr) of
          Left _  -> pure ()
          Right _ -> fail "Expected Left error for non-polynomial"
    , testCase "coefficient of a*(x^2 + 1)^2 + (x^2 + 1) in {x^2 + 1} at j=2 is a" $ do
        let a = mkSymbol "a" :: UnsimplifiedExpr
            ...
            x2Plus1 = (x ** (2 :: UnsimplifiedExpr)) + 1
            expr = a * (x2Plus1 ** (2 :: UnsimplifiedExpr)) + x2Plus1
        coefficientGpe (simplifyOrFail x2Plus1) 2 (simplifyOrFail expr) @?= Right (simplifyOrFail a)
    , testCase "coefficient of a*(x^2 + 1)^2 + (x^2 + 1) in {x^2 + 1} at j=1 is 0" $ do
        let a = mkSymbol "a" :: UnsimplifiedExpr
            x = mkSymbol "x" :: UnsimplifiedExpr
            x2Plus1 = (x ** (2 :: UnsimplifiedExpr)) + 1
            expr = a * (x2Plus1 ** (2 :: UnsimplifiedExpr)) + x2Plus1
        coefficientGpe (simplifyOrFail x2Plus1) 1 (simplifyOrFail expr) @?= Right (simplifyOrFail 0)
    , testCase "coefficient of a*(x^2 + 1)^2 + (x^2 + 1) in {x^2 + 1} at j=0 is x^2 + 1" $ do
        let a = mkSymbol "a" :: UnsimplifiedExpr
            x = mkSymbol "x" :: UnsimplifiedExpr
            x2Plus1 = (x ** (2 :: UnsimplifiedExpr)) + 1
            expr = a * (x2Plus1 ** (2 :: UnsimplifiedExpr)) + x2Plus1
        coefficientGpe (simplifyOrFail x2Plus1) 0 (simplifyOrFail expr) @?= Right (simplifyOrFail x2Plus1)
    , testCase "coefficient of 2*(x^2)^2 + 3*(x^2) in {x^2} at j=2 is 0" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            x2 = x ** (2 :: UnsimplifiedExpr)
            expr = mkNumber 2 * (x2 ** (2 :: UnsimplifiedExpr)) + mkNumber 3 * x2
        coefficientGpe (simplifyOrFail x2) 2 (simplifyOrFail expr) @?= Right (mkNumber 0)
    , testCase "coefficient of 2*(x^2)^2 + 3*(x^2) in {x^2} at j=1 is 3" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            x2 = x ** (2 :: UnsimplifiedExpr)
            expr = mkNumber 2 * (x2 ** (2 :: UnsimplifiedExpr)) + mkNumber 3 * x2
        coefficientGpe (simplifyOrFail x2) 1 (simplifyOrFail expr) @?= Right (mkNumber 3)
    , testCase "coefficient of 2*(x^2)^2 + 3*(x^2) in {x^2} at j=0 is 2*x^4" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            x2 = x ** (2 :: UnsimplifiedExpr)
            expr = mkNumber 2 * (x2 ** (2 :: UnsimplifiedExpr)) + mkNumber 3 * x2
        coefficientGpe (simplifyOrFail x2) 0 (simplifyOrFail expr) @?= Right (simplifyOrFail (mkNumber 2 * (x2 ** (2 :: UnsimplifiedExpr))))
    ]
  ...
  testGroup
    ...
    [ testCase "leading coefficient of a*x^2 + b*x + c in x is a" $ do
        let a = mkSymbol "a" :: UnsimplifiedExpr
            ...
            x = mkSymbol "x" :: UnsimplifiedExpr
            expr = (a * (x ** (2 :: UnsimplifiedExpr))) + (b * x) + c
        leadingCoefficientGpe (simplifyOrFail x) (simplifyOrFail expr) @?= Right (simplifyOrFail a)
    , testCase "leading coefficient of 3*x*y^2 + 5*x^2*y + 7*x + 9 in x is 5*y" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            y = mkSymbol "y" :: UnsimplifiedExpr
            expr = mkNumber 3 * x * (y ** (2 :: UnsimplifiedExpr)) + mkNumber 5 * (x ** (2 :: UnsimplifiedExpr)) * y + mkNumber 7 * x + mkNumber 9
            expected = mkNumber 5 * (y ** (1 :: UnsimplifiedExpr))
        leadingCoefficientGpe (simplifyOrFail x) (simplifyOrFail expr) @?= Right (simplifyOrFail expected)
    ...
    , testCase "leading coefficient of constant 3 is 3" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
        leadingCoefficientGpe (simplifyOrFail x) (simplifyOrFail (mkNumber 3 :: UnsimplifiedExpr)) @?= Right (mkNumber 3)
    , testCase "leading coefficient of zero is 0" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
        leadingCoefficientGpe (simplifyOrFail x) (simplifyOrFail (mkNumber 0 :: UnsimplifiedExpr)) @?= Right (mkNumber 0)
    , testCase "leadingCoefficientGpe errors on non-GPE" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            expr = (x + 1) * (x + 3)
        case leadingCoefficientGpe (simplifyOrFail x) (simplifyOrFail expr) of
          Left UnsupportedOperation{} -> pure ()
          _ -> fail "Expected Left UnsupportedOperation for non-GPE"
    ]
...
coeffVarMonomialTests =
  testGroup
    ...
    [ testCase "constant returns (constant,1)" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
        coeffVarMonomial (HS.fromList [simplifyOrFail x]) (simplifyOrFail (mkNumber 3)) @?= Right (mkNumber 3, mkNumber 1)
    , testCase "a * x^2 returns (a, x^2)" $ do
        let a = mkSymbol "a" :: UnsimplifiedExpr
            x = mkSymbol "x" :: UnsimplifiedExpr
            expr = simplifyOrFail (a * (x ** (2 :: UnsimplifiedExpr)))
        coeffVarMonomial (HS.fromList [simplifyOrFail x]) expr @?= Right (simplifyOrFail a, simplifyOrFail (x ** (2 :: UnsimplifiedExpr)))
    , testCase "3*x*y^2 with vars {x,y} returns (3, x*y^2)" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            y = mkSymbol "y" :: UnsimplifiedExpr
            expr = simplifyOrFail (mkNumber 3 * x * (y ** (2 :: UnsimplifiedExpr)))
        coeffVarMonomial (HS.fromList [simplifyOrFail x, simplifyOrFail y]) expr @?= Right (mkNumber 3, simplifyOrFail (x * (y ** (2 :: UnsimplifiedExpr))))
    , testCase " x^4 with vars {x^2} returns (x^4, 1)" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            expr = simplifyOrFail (x ** (4 :: UnsimplifiedExpr))
        coeffVarMonomial (HS.fromList [simplifyOrFail (x ** (2 :: UnsimplifiedExpr))]) expr @?= Right (simplifyOrFail (x ** (4 :: UnsimplifiedExpr)), mkNumber 1)
    , testCase "variable itself returns (1, x)" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
        coeffVarMonomial (HS.fromList [simplifyOrFail x]) (simplifyOrFail x) @?= Right (mkNumber 1, simplifyOrFail x)
    , testCase "free-of-variable monomial returns (monomial,1)" $ do
        let a = mkSymbol "a" :: UnsimplifiedExpr
            x = mkSymbol "x" :: UnsimplifiedExpr
            expr = simplifyOrFail (mkNumber 3 * a)
        coeffVarMonomial (HS.fromList [simplifyOrFail x]) expr @?= Right (expr, mkNumber 1)
    ]
...
collectTermsTests =
  testGroup
    ...
    [ testCase "Collected form of 2*a*x*y + 3*b*x*y + 4*a*x + 5*b*x in {x, y} is (2*a + 3*b)*x*y + (4*a + 5*b)*x" $ do
        ...
        collectTerms (HS.fromList [simplifyOrFail x, simplifyOrFail y]) (simplifyOrFail expr) @?= Right (simplifyOrFail expected)
    , testCase "Collected form of 2*a*x*y + 3*b*x*y + 4*a*x + 5*b*x in {a, b} is (2*x*y + 4*x)*a + (3*x*y + 5*x)*b" $ do
        ...
        collectTerms (HS.fromList [simplifyOrFail a, simplifyOrFail b]) (simplifyOrFail expr) @?= Right (simplifyOrFail expected)
    , testCase "Collect simple like terms 2*x + 3*x + 4 in {x} -> 5*x + 4" $ do
        ...
        collectTerms (HS.fromList [simplifyOrFail x]) (simplifyOrFail expr) @?= Right (simplifyOrFail expected)
    , testCase "Collect cancelling terms a*x - a*x + b in {x} -> b" $ do
        let a = mkSymbol "a" :: UnsimplifiedExpr
            b = mkSymbol "b" :: UnsimplifiedExpr
            x = mkSymbol "x" :: UnsimplifiedExpr
            expr = a * x + (mkNumber (-1) * a * x) + b
        collectTerms (HS.fromList [simplifyOrFail x]) (simplifyOrFail expr) @?= Right (simplifyOrFail b)
    , testCase "Collect with generalized variable x^2: 2*(x^2)^2 + 3*(x^2) in {x^2} keeps distinct parts" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            x2 = x ** (2 :: UnsimplifiedExpr)
            expr = mkNumber 2 * (x2 ** (2 :: UnsimplifiedExpr)) + mkNumber 3 * x2
        collectTerms (HS.fromList [simplifyOrFail x2]) (simplifyOrFail expr) @?= Right (simplifyOrFail expr)
    , testCase "Collected form of a*x*sin(x)*x + b in {x} does not exist because it is not a polynomial in x" $ do
        ...
        case collectTerms (HS.fromList [simplifyOrFail x]) (simplifyOrFail expr) of
          Left _  -> pure ()
          Right _ -> fail "Expected Left error for non-polynomial"
    , testCase "Collected form of 2*a*x + 3*a*x*b in {a, x} is (2 + 3*b)*a*x" $ do
        ...
        collectTerms (HS.fromList [simplifyOrFail a, simplifyOrFail x]) (simplifyOrFail expr) @?= Right (simplifyOrFail expected)
    ]
...
rationalGreTests =
  testGroup
    ...
    [ testCase "(x^2 + 1)/(2*x + 3) is rational in {x}" $ do
        ...
        isRationalGre (HS.fromList [simplifyOrFail x]) (simplifyOrFail expr) @?= True
    , testCase "1/x + 1/y is not rational in {x,y}" $ do
        let x = mkSymbol "x" :: UnsimplifiedExpr
            y = mkSymbol "y" :: UnsimplifiedExpr
            expr = mkNumber 1 / x + mkNumber 1 / y
        isRationalGre (HS.fromList [simplifyOrFail x, simplifyOrFail y]) (simplifyOrFail expr) @?= False
    ]
